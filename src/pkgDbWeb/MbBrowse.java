...
public class MbBrowse implements Serializable {
	...
	public void insert(ActionEvent ae) {
		...
		try {
			// if( ps == null ){
			String sQl = "INSERT INTO Kunde( " + "KundenID, Vorname, Nachname, Adresse, Postleitzahl, Telefonnummer ) "
					+ "VALUES ( ?, ?, ?, ?, ?, ? )";
		...
		public void update(ActionEvent ae) {
			...
			try {
				PreparedStatement ps = con.prepareStatement(
						"UPDATE Kunde SET " + "Vorname = ?, Nachname= ?, Adresse= ?, Postleitzahl=?, Telefonnummer=? "
								+ "WHERE KundenID = ?");
				...
				if (n == 1) {
					...
					FacesContext.getCurrentInstance().addMessage(null,
							new FacesMessage(FacesMessage.SEVERITY_INFO, "O. K.",
